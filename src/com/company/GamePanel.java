...
import java.util.Random;
    ...
    public class GamePanel extends JPanel implements ActionListener {       //GamePanel class extends properties of JPanel class and implements properties of interface ActionListener also
        ...
        int snake_length = 1;                             //initially set blocks for snake body
        ...
        GamePanel() {
            ...
            this.setFocusable(true); //it mainly activates the components inside ur panel by passing boolean value inside it
            this.addKeyListener(new MyKeyAdapter()); //we create a class MyKeyAdapter which extends the properties from KeyAdapter class from java and adds a keylistener which took the response.
            startGame();
        }
        ...
        public void paintComponent(Graphics g) { //we take a parameter g from Graphics class used in drawing the graphical structure in java
        ...
        public void draw(Graphics g) {
            ...
            if (running) {  //this method first checks for running status of our game that if it is true then we can move inside the if block
            ...
            for (int i = 0; i < SCREEN_HEIGHT / UNIT_SIZE; i++) {  //this for loop is used for making our panel looks like grid that helps us to then understanding how our program works
                ...
                g.drawLine(0, i * UNIT_SIZE, SCREEN_WIDTH, i * UNIT_SIZE);  //where it draws line between these two points that we pass in our method as parameters
            }*/
                ...
                FontMetrics metrics = getFontMetrics(g.getFont()); //FontMetrices class inside awt package is used to encapsulate a given font and here we declare "metrics" as FontMatrices and then get font we gave to g Graphics
        ...
        public void checkFood() {
            if ((x[0] == snakeFoodX) && (y[0] == snakeFoodY)) { //here as x and y array includes the dimensions of our snake therefore if dimension of head of our snake is equal to the position of food then it must eat it
